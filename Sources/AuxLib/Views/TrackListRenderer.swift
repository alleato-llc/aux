...
public struct TrackListRenderer {
    public static func render(
        ...
        if state.currentTrack != nil {
            title = "\(album.name) by \(album.artist) | \(state.musicIcon)"
        } else {
        ...
        // Album info line: "9 tracks / 38:20 [1978] [Country Rock] | FLAC/16-bit/44.1kHz"
        var infoText = "  \(album.trackCount) tracks / \(album.formattedDuration)"
        if let year = album.year { infoText += " [\(year)]" }
        if let genre = album.genre { infoText += " [\(genre)]" }
        if let format = album.formatDescription { infoText += " | \(format)" }
        buffer.write(infoText, x: innerArea.x, y: innerArea.y, style: theme.secondary)

        // Track table below info line, inset for padding
        let tableArea = Rect(
            x: innerArea.x + 1,
            y: innerArea.y + 1,
            width: max(0, innerArea.width - 2),
